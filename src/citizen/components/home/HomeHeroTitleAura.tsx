...
/** Aura detrás del título — nodos/líneas suaves (interactivo visual, sin texto). */
    ...
    <div className={`hero-title-aura ${reduceMotion ? '' : 'is-alive'}`} aria-hidden>
      <svg className="hero-title-aura-svg" viewBox="0 0 640 220" fill="none">
        ...
        <path
          ...
          opacity="0.7"
        />
        <circle className="hero-title-aura-node" cx="120" cy="78" r="4" />
        <circle className="hero-title-aura-node" cx="320" cy="90" r="5" />
        <circle className="hero-title-aura-node" cx="480" cy="120" r="4" />
        <circle className="hero-title-aura-node" cx="560" cy="88" r="3.5" />
      </svg>
    </div>
  );
}
