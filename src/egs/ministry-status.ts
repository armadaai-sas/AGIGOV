...
export function deriveEgsMinistryStatus(
  ...
  if (!health) {
    return {
      ...
      semaphore: 'amber',
      blockReason: 'Sin telemetría fiscal provisionada',
  ...
  if (tenant?.onboardingStatus === 'baseline_pending') {
    return {
      ...
      semaphore: 'amber',
      blockReason: 'Acta baseline pendiente de ratificación multi-sig',
      tenantSlug: tenant.slug,
      primaryAction: {
        id: 'ratify_baseline',
        label: 'Ratificar acta baseline',
        enabled: true,
        href: '/institucional/piloto',
      },
      secondaryActions,
  ...
  if (health.releaseCount === 0) {
    return {
      ...
      semaphore: 'amber',
      blockReason: 'Faltan hitos verificados para cierre trimestral',
      tenantSlug: tenant?.slug ?? null,
      primaryAction: {
        id: 'ingest_milestones',
        label: 'Ingestar hitos del piloto',
        enabled: true,
        href: '/institucional/piloto',
      },
      secondaryActions,
