# AGIGOV — Documentación institucional

**AGIGOV** (*Governanza de Inteligencia General Autónoma*) es el modelo genérico de Estado–Política–Economía que este repositorio implementa como infraestructura soberana.

## Índice

| Documento | Contenido |
|-----------|-----------|
| [CONCEPTO.md](./CONCEPTO.md) | Qué es AGIGOV, pilares, audiencias |
| **[CARTA-AGIGOV-VEN.md](./CARTA-AGIGOV-VEN.md)** | **Carta institucional v0.1 (33 artículos)** |
| [PLAN.md](./PLAN.md) | Plan de ejecución por fases (P0–P6) |
| [AGIGOV-VEN.md](./AGIGOV-VEN.md) | Implementación nacional Venezuela |
| [AGIGOV-SBX.md](./AGIGOV-SBX.md) | Sandbox interop (ISO SBX + VEN) |
| **[CARTA-AGIGOV-SBX.md](./CARTA-AGIGOV-SBX.md)** | **Carta sandbox adhesión red** |
| [ONBOARDING-GOBIERNOS.md](./ONBOARDING-GOBIERNOS.md) | Registro y conexión de gobiernos |
| [../SOVEREIGN-CONFIG.md](../SOVEREIGN-CONFIG.md) | País, moneda, idioma (VEN/COL) |
| [POLITICA-2.0.md](./POLITICA-2.0.md) | Política tradicional vs Política AGI 2.0 |
| **[POLITICA-PARTICIPACION.md](./POLITICA-PARTICIPACION.md)** | **Uso de propuestas, aportes y campañas (piloto)** |
| [ECONOMIA-DAO.md](./ECONOMIA-DAO.md) | Tokens, DAO, prosperidad compartida |
| [CNE-TOKENIZADO.md](./CNE-TOKENIZADO.md) | Centro Nacional Electoral y voto seguro |
| [SEGURIDAD-PQC.md](./SEGURIDAD-PQC.md) | Seguridad, PQC, ciberdefensa |
| [CONTRIBUCION.md](./CONTRIBUCION.md) | Puentes para personas, IA y gobiernos |
| **[MODEL-MANIFEST-v1.md](./MODEL-MANIFEST-v1.md)** | **Contrato de modelo comunitario (SDK / rev-share)** |
| **[PLAN-PULIDO-G1-G20.md](./PLAN-PULIDO-G1-G20.md)** | **Pulido UX/nav G1–G20 · Scrum · GO/NO-GO** |
| **[ESCRUTINIO-VENTANAS.md](./ESCRUTINIO-VENTANAS.md)** | **Hallazgos GO/NO-GO por ventana (live)** |
| [revisiones/](./revisiones/) | Informes legal-político (`state-legal-political`) |
| **[ROADMAP-EJECUCION.md](./ROADMAP-EJECUCION.md)** | **Pasos 1–14 ejecutables uno a uno** |

## Relación con Armada VZLA

| Capa | Nombre público | Rol técnico |
|------|----------------|-------------|
| Modelo | **AGIGOV** | Marco genérico replicable |
| Implementación VE | **AGIGOV-VEN** | Adaptación Venezuela |
| Infraestructura | **Armada VZLA** | Código, ledger, agentes, PWA |

Cualquier gobierno puede desplegar **AGIGOV-[ISO]** sobre la misma red e infraestructura base.
